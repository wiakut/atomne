import { useGame } from '../state/GameContext';
import { GlowButton } from '../components/GlowButton';
import { ScreenWrapper } from '../components/ScreenWrapper';
import { ProgressBar } from '../components/ProgressBar';
import { ROUNDS } from '../utils/constants';

export function LeaderboardScreen() {
  const { state, dispatch } = useGame();
  const isLastRound = state.currentRoundIndex + 1 >= state.activeRounds.length;
  if (state.teams.length === 0) return null;

  const teamScores = state.teams.map(t => ({
    ...t,
    total: t.scores.reduce((a, b) => a + b, 0),
  })).sort((a, b) => b.total - a.total);

  const maxScore = Math.max(1, teamScores[0]?.total ?? 1);

  // Rounds played so far (up to and including current)
  const playedRounds = state.activeRounds.slice(0, state.currentRoundIndex + 1);

  return (
    <ScreenWrapper className="justify-center items-center gap-5">
      <button
        onClick={() => dispatch({ type: 'NEW_GAME' })}
        className="absolute top-5 right-5 text-white/30 hover:text-white/60 text-sm transition-colors"
      >
        Вийти
      </button>

      <h2 className="text-2xl font-bold text-cyan text-glow-cyan">
        Результати раунду
      </h2>
      <div className="text-white/40 text-sm">
        {ROUNDS[(state.activeRounds[state.currentRoundIndex] ?? 1) - 1].emoji}{' '}
        {ROUNDS[(state.activeRounds[state.currentRoundIndex] ?? 1) - 1].name}
      </div>

      <div className="w-full max-w-sm flex flex-col gap-3">
        {teamScores.map((team, i) => {
          return (
            <div key={team.id} className="bg-white/[0.03] rounded-xl p-3">
              <div className="flex justify-between items-center mb-1">
                <span className={i === 0 ? 'text-cyan font-bold' : 'text-white/80'}>
                  {i === 0 && teamScores[0].total > 0 ? '👑 ' : ''}{team.name}
                </span>
                <span className="text-cyan font-bold text-lg">{team.total}</span>
              </div>
              <ProgressBar value={team.total} max={maxScore} color={i === 0 ? 'cyan' : 'pink'} />
              <div className="flex gap-2 mt-2 flex-wrap">
                {playedRounds.map((roundId, ri) => {
                  const round = ROUNDS[roundId - 1];
                  const score = team.scores[ri] ?? 0;
                  const isCurrent = ri === state.currentRoundIndex;
                  return (
                    <span
                      key={roundId}
                      className={`text-xs px-2 py-0.5 rounded ${
                        isCurrent
                          ? 'bg-cyan/15 text-cyan font-semibold'
                          : 'bg-white/5 text-white/40'
                      }`}
                    >
                      {round.emoji} {score}
                    </span>
                  );
                })}
              </div>
            </div>
          );
        })}
      </div>

      <GlowButton
        onClick={() => dispatch({ type: 'NEXT_ROUND' })}
        variant="filled"
        className="mt-2 min-w-48"
      >
        {isLastRound ? 'Фiнальнi результати' : 'Наступний раунд'}
      </GlowButton>
    </ScreenWrapper>
  );
}
